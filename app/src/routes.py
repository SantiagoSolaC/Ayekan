...
resident_context = {"active_item": "resident", "user": current_user}
...
def resident_new():
    if request.method == "GET":
        return render_template("resident_new.html", **resident_context)
    elif request.method == "POST":
...
medication_context = {"active_item": "medication", "user": current_user}
...
def medication_new():
    if request.method == "GET":
        return render_template("medication_new.html", **medication_context)
    elif request.method == "POST":
...
def medication_details(id):
    ...
    return render_template("medication_details.html", medication = medication, **medication_context)
...
def medication_edit(id):
    if request.method == "GET":
        medication = Medication.query.get(int(id))
        return render_template( "medication_edit.html", medication = medication, **medication_context)
    elif request.method == "POST":
...
prescription_context = {"active_item": "prescription", "user": current_user}
...
def prescription_search():
    if request.method == "GET":
        return render_template("prescription_search.html", **prescription_context)
    elif request.method == "POST":
        if request.form.get("select_field") == "resident_name":
            resident_list = resident_search_by_value("name", request.form.get("imput_field"))
            return render_template("/prescription_list_from_resident.html", resident_list = resident_list, **prescription_context)
        elif request.form.get("select_field") == "medication_name":
            medication_list = medication_search_by_value("commercial_name", request.form.get("imput_field"))
            return render_template("prescription_list_from_medication.html", medication_list = medication_list, **prescription_context)
...
def prescription_list_from_medication(medication_id):
    ...
    prescription_list = db.session.query(Prescription).join(Stock).join(medication_stock).join(Medication).filter(Medication.id == medication_id).all()
    return render_template("prescription_list.html", medication = medication, prescription_list = prescription_list, **prescription_context)
...
def prescription_details(prescription_id):
    ...
    medication = prescription.stocks[0].medications[0]
    return render_template("prescription_details.html", prescription = prescription, medication = medication, **prescription_context)
...
def prescription_edit(prescription_id):
    if request.method == "GET":
        ...
        return render_template("prescription_edit.html", prescription = prescription, **prescription_context)
    elif request.method == "POST":
...
def prescription_new():
    if request.method == "GET":
        medication_list = Medication.query.all()
        resident_list = Resident.query.all()
        return render_template("prescription_new.html", active_item="prescription", user=current_user, medication_list=medication_list, resident_list=resident_list)
    elif request.method == "POST":
