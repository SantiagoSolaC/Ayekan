...
def resident_search():
    if request.method == "GET":
        return render_template("resident_search.html", active_item = "resident", user = current_user)
        ...
        return render_template("resident_list.html", resident_list = resident_list, active_item = "resident", user = current_user)
...
def resident_details(id):
    ...
    resident.to_show_in_html()
    return render_template("resident_details.html", resident = resident, active_item = "resident", user = current_user)

@views.route("/resident_creation", methods = ["GET", "POST"])
@login_required
def resident_creation():
    if request.method == "GET":
        return render_template("resident_creation.html", active_item = "resident", user = current_user)
    elif request.method == "POST":
        resident = resident_from_dictionary(request.form)
...
def resident_edition(id):
    if request.method == "GET":
        resident = Resident.query.get(int(id))
        resident.to_show_in_html()
        return render_template("resident_edition.html", resident = resident, active_item = "resident", user = current_user)
    elif request.method == "POST":
        resident_dictionary(request.form)
        return redirect(url_for("views.resident_search"))
...
def medication_search():
    if request.method == "GET":
        return render_template("medication_search.html", active_item = "medication", user = current_user)
    elif request.method == "POST":
        ...
        return render_template("medication_list.html", medication_list = medication_list, active_item = "medication", user = current_user)
...
def prescription_search():
    if request.method == "GET":
        return render_template("prescription_search.html", active_item = "prescription", user = current_user)
    elif request.method == "POST":
        prescription_list = prescription_search_by_value(request.form.get("select_field"), request.form.get("imput_field"))
        for prescription in prescription_list:
            prescription.to_show_in_html()
        return render_template("prescription_list.html", prescription_list = prescription_list, active_item = "prescription", user = current_user)
