...
class Resident:
    def __init__(self, resident_dictionary):
        self.resident_id = resident_dictionary.get('resident_id')
        self.status = resident_dictionary.get('status')
        self.admission_date = resident_dictionary.get('admission_date')
        self.name = resident_dictionary.get('name')
        self.last_name = resident_dictionary.get('last_name')
        self.birth_date = resident_dictionary.get('birth_date')
        self.age = resident_dictionary.get('age')
        self.gender = resident_dictionary.get('gender')
        self.citizenship = resident_dictionary.get('citizenship')
        self.marital_status = resident_dictionary.get('marital_status')
        self.address = resident_dictionary.get('address')
        self.city = resident_dictionary.get('city')
        self.id_type = resident_dictionary.get('id_type')
        self.id_number = resident_dictionary.get('id_number')
        self.prepaid = resident_dictionary.get('prepaid')
        self.affiliation_number = resident_dictionary.get('affiliation_number')
        self.nickname = resident_dictionary.get('nickname')
...
def get_resident_by_id(resident_id):
    with sqlite3.connect("./ayekan.db") as conn:
        cursor = conn.cursor()
        cursor.execute(f'SELECT * FROM residents WHERE resident_id="{resident_id}";')
        result = cursor.fetchone()
        resident_dictionary = create_resident_dictionary(result)
...
def get_resident_list_by_value(field_name, value):
    with sqlite3.connect("./ayekan.db") as conn:
        cursor = conn.cursor()
        cursor.execute(f'SELECT * FROM residents WHERE {field_name} LIKE "%{value}%";')
        result = cursor.fetchall()
        residents_list = []
        for resident_tuple in result:
            resident_dictionary = create_resident_dictionary(resident_tuple)
            resident = Resident(resident_dictionary)
